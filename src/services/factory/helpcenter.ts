...
export class HelpCenter implements IZendeskImport {
		...
		while (next) {
			...
			if (result.next_page) {
				...
				if (this.hasIncremental()) {
					this._page = 0;
					if (result.count > 0) {
						console.log('result.end_time', result.end_time);
						this._incremental = result.end_time;
					}
				} else {
					const u = new URLSearchParams(url);
					this._page = u.get('page') ? +u.get('page') : 0;
					this._incremental = '';
				}
				next = false;
			}
	...
	public async saveDb () {
		...
		console.log('content', this._content.length, this._config.id, this._incremental, this._page);
